...
function DogNameProvider({children}) {
  const [state, setState] = React.useState({dogName: ''})
...
function useDogNameContext() {
  ...
  if (!context) {
    throw new Error('useDogNameContext must be used within the DogProvider')
  }
  return context
}
...
function DogNameInput() {
  ...
  //const [state, setState] = React.useState({dogName: ''})
  const [state, setState] = useDogNameContext()
  //const dispatch = useAppDispatch()
  const {dogName} = state
