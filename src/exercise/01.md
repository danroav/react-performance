...
import('module path') can import libraries dynamically, if they are marked with module just the name of the module is needed otherwise the full path is required

coverage is a browser developer tool that can help track the usage of code in the current request/response.

bundlers split code in files with the goal to improve performance of files and code needed by the user but sometimes its not enough or it complicates it.

React.lazy allows to import libraries or components when they are called for the first time. It complements with React.Suspense component that encapsulates components obtained the lazy way.

To preload a component is a good practice to start the dynamic import before calling React.lazy without blocking the rendering of the parent or main components 
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Performance%20%E2%9A%A1&e=01%3A%20Code%20splitting&em=a.b%40example.com).
